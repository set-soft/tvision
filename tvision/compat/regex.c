...
static void
insert_op1 ( re_opcode_t op, unsigned char *loc, int arg,
    unsigned char *end );
...
static boolean
at_begline_loc_p ( const char *pattern, const char *p,
    reg_syntax_t syntax );
...
compile_range ( const char **p_ptr, const char *pend,
...
static boolean
at_endline_loc_p (const char *p, const char *pend,
    int syntax )
{
  const char *next = p;
...
#define PUSH_FAILURE_ITEM(item)                                         \
  fail_stack.stack[fail_stack.avail++] = (fail_stack_elt_t) (ipointer) item
...
{                                                                       \
  ...
  high_reg = (uipointer) POP_FAILURE_ITEM ();                           \
  DEBUG_PRINT2 ("  Popping high active reg: %d\n", high_reg);           \
                                                                        \
  low_reg = (uipointer) POP_FAILURE_ITEM ();                            \
                      ...
                      for (r = *p; r < *p + *(p + 1); r++)
                        {
                          ...
                          /* xx why this test?  */
                          if ((ipointer) old_regend[r] >= (ipointer) regstart[r])
